...
var errCloseConns = errors.New("http2: CloseConnections called")

// CloseConnections closes any connections which were previously
// connected from previous requests.
func (t *Transport) CloseConnections(f func(addr net.Addr, idle bool) bool) {
	if cp, ok := t.connPool().(*clientConnPool); ok {
		...
		for _, vv := range cp.conns {
			for _, cc := range vv {
				cc.mu.Lock()
				if f(cc.tconn.RemoteAddr(), len(cc.streams) == 0) {
					cc.closed = true
				}
				cc.mu.Unlock()
				if cc.closed {
